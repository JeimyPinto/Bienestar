...
export default function RequestsForm(props: RequestsFormProps) {
    ...
    async function handleSubmit(event: React.FormEvent) {
        ...
        try {
            let response: { error: boolean; message?: string } | undefined;
            if (mode === "create") {
    ...
    if (!token || !user) {
        return (
            <>
                <div className="fixed inset-0 bg-azul-marino/60 backdrop-blur-md z-[9998]"></div>
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 w-full h-full">
                    <div className="bg-white rounded-2xl shadow-2xl border border-azul-cielo/20 w-full max-w-md overflow-hidden">
    ...
    return (
        <>
            {/* Backdrop manual con z-index altísimo */}
            <div className="fixed inset-0 bg-azul-marino/60 backdrop-blur-md z-[9998]"></div>

            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 w-full h-full">
                <div className="bg-white rounded-2xl shadow-2xl border border-azul-cielo/20 w-full max-w-5xl max-h-[90vh] overflow-y-auto overflow-x-hidden relative">
